...
getjqtversion=: 3 : 0
suffix=. (IFUNIX>'/'e.LIBFILE)#'-9.04'  NB. deb install
...
NB. busybox tar is faster than jtar
  ...
  if. notarcmd do.
    ...
    e=. shellcmd 'tar ',((IFIOS+:UNAME-:'Android')#(((<UNAME)e.'Darwin';'OpenBSD'){::'--no-same-owner --no-same-permissions';'- o -p')),' -xzf ',file,' -C ',dir
  end.
